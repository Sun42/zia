...
QTreeWidgetItem*	ZiaPluginLoader::find(const QString& pluginName) const
{
	QList<QTreeWidgetItem*>	found(this->treeWidget->findItems(pluginName, Qt::MatchExactly));
	if (found.size())
	{
		return (found.at(0));
	}
...
void	ZiaPluginLoader::removeItem(const QString& pluginName) const
{
	QTreeWidgetItem*	childToRemove = find(pluginName);
	if (childToRemove != 0)
	{
...
void	ZiaPluginLoader::loadPlugin(void)
{
	...
	{
		// DEBUG
		std::cerr << fileNames.at(i).toStdString() << std::endl;
...
void	ZiaPluginLoader::refreshPlugin(void)
{
	const QTreeWidgetItem*	currentItem = this->treeWidget->currentItem();
...
{
	std::cerr << "processLoginState" << std::endl;
	if (true == success)
	{
...
void	ZiaPluginLoader::processUnloadPluginState(bool success, const QString& explanation)
{
	if (true == success)
	{
	...
	{
		this->statusBar()->showMessage(tr("Unloading failed: ") + explanation, 3000);
	}
}
...
void	ZiaPluginLoader::connectActions(void)
{
	...
	QObject::connect(this->actionRefresh, SIGNAL(triggered()), SLOT(refreshPlugin()));
	// Options
