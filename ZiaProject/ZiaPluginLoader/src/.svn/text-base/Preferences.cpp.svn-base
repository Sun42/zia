...
void	Preferences::readSettings(void)
{
	...
	this->_autoConnect = settings.value("autoconnect", bool(true)).toBool();
	settings.endGroup();
}
...
void	Preferences::update(void)
{
	...
	if (true == this->_autoConnect)
		this->checkBoxAutoConnect->setCheckState(Qt::Checked);
	else
		this->checkBoxAutoConnect->setCheckState(Qt::Unchecked);
}
...
void	Preferences::save(void)
{
	...
	if (Qt::Checked == this->checkBoxAutoConnect->checkState())
