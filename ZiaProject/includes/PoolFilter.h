...
class	PoolFilter
{
 public:
  ...
  PoolFilter(const zia::filter::IFilter*, Plugin*, HookType);
  ~PoolFilter(void);
  ...
  ActionOnPool	releaseInstance(zia::filter::IFilter*); /*!< relache l'instance dans le pool */

 private:
  Plugin*			_plugin; /*!< Plugin d ou provient le filtre */
  const zia::filter::IFilter*	_filter; /*!< Filtre qui permet de construire les instance du pool*/
  HookType			_numHook;
