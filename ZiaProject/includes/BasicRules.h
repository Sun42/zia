...
/*! \struct BasicRules
 * \brief Regles basiques de la RFC 2616.
 *
 * Les regles ecrites en Boost Spirit les plus simples sont definies ici.
 */
...
struct	BasicRules
{
  BasicRules(void)
  {
    ...
    TEXT		= (LWS | (CTL_N & OCTET)); // <any OCTET except CTLs, but including LWS>
    HEX			= xdigit_p;
    ...
    separators_n	= (~ch_p('(') & ~ch_p(')') & ~ch_p('<') & ~ch_p('>') &
			   ...
			   ~ch_p(']') & ~ch_p('?') & ~ch_p('=') & ~ch_p('{') &
			   ~ch_p('}') & SP_N & HT_N);
    token		= +(CTL_N  & separators_n & CHAR); // 1*<any CHAR except CTLs or separators>
