...
class	PluginManager
{
 public:
  ...
  unsigned char		loadPlugin(std::string const & path); // load the dll/so at the specific path
  void			loadPlugins(std::string const & directory); // load the dll/so at the specific directory
